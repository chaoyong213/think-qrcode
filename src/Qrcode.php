...
class Qrcode
{
     ...
     * @return $this
     */
    public function png(string $url, string $outfile = '', int $size = 5, string $level = 'H')
    {
        if (!$outfile) {
     ...
     * @return string
     */
    public function getPath(bool $ds = true): string
    {
        if ($ds) {
            return '/' . $this->outfile;
        } else {
...
//        $this->outfile = $this->cache_dir . '/' . time() . '.png';
     ...
     * @return $this
     */
    public function background(int $x = 200, int $y = 500, string $dst_path = '')
    {
        if ($dst_path == '') {
...
//        $this->outfile = $this->cache_dir . '/' . time() . '.png';
     ...
     * @return $this
     */
    public function text(string $text, float $size, array $locate = [], string $color = '#00000000', string $font = 'simsun.ttc', int $offset = 0, int $angle = 0)
    {
